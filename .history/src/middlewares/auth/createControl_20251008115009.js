...
const createControl = async (req, res, next) => {
  ...
  try {
    ...
    const creatorPermissions = permission.find((doc) => doc.role === newEmployeeRole).permission;
    console.log(creatorRole, newEmployeeRole, newEmployeeRole <= creatorRole);
    if (newEmployeeRole <= creatorRole && !creatorPermissions.includes("create_employee")) {
