...
const authToken = async (req, res, next) => {
  ...
  try {
    ...
    if ((await Employee.countDocuments()) === 0) {
      req.user = await SuperAdminCollection.findOne({ email: decoded.id });
      console.log(await SuperAdminCollection.findOne({ email: decoded.id }));
    } else {
      // Attach user to request (excluding password)
      req.user = await Employee.findOne({ email: decoded.id });
    }
